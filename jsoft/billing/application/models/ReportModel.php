...
public function dashboardDataDetails(){
		...
		$this->db->where('icw_shopId0317',$shopId);
		$queryTotalSales=$this->db->get('icw_invoice_0317')->result();
		$queryTotalSales=count($queryTotalSales);
		
		//$this->db->where('icw_fk_shop_id',$shopId);
		$TotalRegistration=$this->db->get('icw_clients_0317')->result();
		$TotalRegistration=count($TotalRegistration);
...
public function get_invoice_report($id){
			$settingDetails=$this->session->userdata('settingDetails');
			$shopId=$settingDetails->icw_shopId0317;
		    ...
		    if($id){
		        $this->db->where('c.icw_invoId0317',$id);
