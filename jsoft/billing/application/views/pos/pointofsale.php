...
<div class="row" style="margin: 12px;">
	<span class="col-md-3 logo_span"> <span class="col-md-2"> <?php $settingDetails=$this->session->userdata('settingDetails');
	if($settingDetails->icw_shop_bLogo_0317){
		?> <img
			src="<?php echo base_url();?>uploads/shop-logo/<?php echo $settingDetails->icw_shop_bLogo_0317;?>"
			class="logo_img" width="70px" height="70px;"
			alt="<?php echo ucwords($settingDetails->icw_shop_name0317);?>" /> <?php }else{ ?>
			<img src="<?php echo base_url();?>assets/images/profile-icon.png"
			class="logo_img" width="70px" height="70px;"
			alt="<?php echo ucwords($settingDetails->icw_shop_name0317);?>" /> <?php } ?>

	</span> 
	<span class="col-md-10 adr_span">
			<h5>
			<?php echo ucwords($settingDetails->icw_shop_name0317);?>
			</h5>
			<h6>
			<?php echo ucwords($settingDetails->icw_shop_Address0317);?>
			</h6> </span> </span> <span class="col-md-3"> <a
		class="btn btn-primary btn-xs" title="Dashboard"
		href="<?php echo site_url('mainController/dashboard');?>"><i
			class="fa fa-arrow-left"
			style="font-size: 20px; cursor: pointer; margin-top: 0px;"></i>
	</a> <a data-toggle="modal" title="cliet registration"
		data-backdrop="static" data-keyboard="false" data-target="#jayModal"
		id="j_form" class="btn btn-primary btn-xs" accesskey="1">New Client</a>&nbsp;<a
		data-toggle="modal" title="cliet search" data-backdrop="static"
		data-keyboard="false" data-target="#searchModal" id=""
		class="btn btn-primary btn-xs" accesskey="2">Client Search</a> <a
		href="javaScript:void(0)" class="btn btn-primary btn-xs"
		onclick="refreshPosBlock('')" accesskey="a">Add Product</a> </span> <span
		class="col-md-2"><input type="text" class="form-control" id="uMob"
		accesskey="3" maxlength=10 placeholder="Mobile no.">
	</span> <span class=""><i
		class="fa fa-question-circle-o pull-right text-primary"
		data-toggle="modal" data-target="#helpModal" accesskey="h"
		style="font-size: 20px; cursor: pointer; margin-left: 20px;"
		title="keyboard shortcut keys" aria-hidden="true"></i>
	</span> <span class=""><a title="Dashboard"
		href="<?php echo site_url('mainController/dashboard');?>"><i
			class="fa fa-dashboard pull-right"
			style="font-size: 20px; cursor: pointer; margin-top: 0px;"></i>
	</a>
	</span> <span class=""><a title="Print"
		href="<?php echo site_url('report/invoice_report');?>"><i
			class="fa fa-print text-success pull-right" aria-hidden="true"
			style="font-size: 20px; cursor: pointer; margin-top: 0px;"></i>
	</a>
	</span>

	<?php if($settingDetails->icw_shop_type0317==5){ ?>
	<span class="dropdown notifications-menu open pull-right" id=""> <a
		href="#" data-toggle="modal" data-keyboard="false" title="Expiry Notification"
		data-target="#expireModal" aria-expanded="true"> <i
			class="fa fa-bell-o" style="font-size: 20px;"></i> <span
			class="label label-warning"><?php if($queryExpireDate){echo count($queryExpireDate);}?>
		</span> </a> </span>
		<?php } ?>
</div>
</section>
...
<div class="row1">
	<div class="col-md-12 box box-info">
		...
		<form name="frmInvoice" method="post" id="frmInvoice"
			...
			?>
			<input type="hidden" name="customerMob" id="customerMob"> <input
				type="hidden" name="client_id" id="client_id"> <input type="hidden"
			...
			<caption class="tblCaption">
				<span class="client_nameSpn spanPositon"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span
					class="client_mobSpn spanPositon1"></span>
			</caption>
			<div id="pos_ref_blk">
...
<!-- Modal -->
<div class="modal"
	id="jayModal" tabindex="-1" role="dialog"
							...
							<div class="col-md-11 box-body"
								...
								<div class="form-group">
									<label for="cBusinessTitle" class="col-sm-4 control-label">Business
										Title</label>
									<div class="col-sm-6">
										<input type="text" class="form-control" name="cBusinessTitle"
											id="cBusinessTitle" value="" placeholder="Business Title">
									</div>
								</div>
								<div class="form-group">
									<label for="cGrn" class="col-sm-4 control-label">GRN</label>
									<div class="col-sm-6">
										<input type="text" class="form-control" name="cGrn" id="cGrn"
											value="" placeholder="GRN">
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label for="productDescription" class="col-sm-4 control-label">Upload
										Image</label>
									<div class="col-sm-6">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<h3 class="" style="margin: 20x;">
							<input class="searchClnt form-control" id="clntName" type="text"
				...
				<div class="row">
					<div class="col-md-12">
						<table class="table" border="none">
							...
							<tr>
								<td>A</td>
								<td>Add Product Row</td>
							</tr>
							<tr>
								<td>X</td>
								<td>Remove Product Row</td>
							</tr>
							<tr>
								<td>H</td>
								<td>Open help desk</td>
							</tr>
						</table>
						<table class="table" border="none">
							<tbody>
								...
								<tr>
									<td>Opera</td>
									<td colspan="3">Opera 15 or newer: [Alt] + <em>accesskey<br> </em>Opera
										12.1 or older: [Shift] [Esc] + <em>accesskey</em></td>
								</tr>
							</tbody>
						</table>
					</div>
...
<div class="modal" id="printInvoModal" data-backdrop="static">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-body print-invoDiv"></div>
			<div class="modal-footer">
				<button type="button" class="btn btn-info col-md-offset-4 printBtn"
					id="printBtn" onclick="PrintElem('.printArea');">Print</button>
			</div>
			<!-- /.modal-content -->
		</div>
		<!-- /.modal-dialog -->
...
<div class="modal" id="expireModal" data-backdrop="static">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header bg-primary">
				<button type="button" class="close" data-dismiss="modal"
				...
				<h4 class="modal-title">Expiry date of medicine by batch no</h4>
			</div>
			<div class="modal-body" style="height: 300px; overflow: scroll;">
				<table id="example" style="table-layout:fixed"
					...
					<tbody>
					<?php $i=1; foreach($queryExpireDate as $v){ ?>
						<tr>
							<td><?php echo $i++;?></td>
						...
						$today=date('Y-m-d');
						if($today >= $expireDate){
							echo '<span class="text-danger">'.getDateFormat_m($v->date) . '</span>';
						}else{
							echo '<span class="text-success">'.getDateFormat_m($v->date).'</span>';
						}?></td>
						</tr>
						<?php } ?>
					</tbody>
