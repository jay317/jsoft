...
{ 
		 ...
		 $imageSave= $_FILES["image"]["name"];
		 if($imageSave !=''){
		 	$imageSave=$imageSave;
		 }else{
		 	$imageSave=$this->input->post('hidden_image');
		 }
		$data=array(
	...
	if($image!=''){
	  ...
	  if ( ! $this->upload->do_upload()){
        $this->session->set_flashdata('message_error', 'Client Not Added cause of image, please select image!');
	...
	if($action='update')
	{ 
		$result=$this->ClientModel->addClient($id,$data);
			if($result == TRUE)
			{
...
function deleteClient(){
	$id =$this->input->post('id');
		$res=$this->ClientModel->deleteClient($id);
